...
alter table public.reward_rules
  ...
  add column if not exists max_booking_coverage numeric check (max_booking_coverage is null or (max_booking_coverage > 0 and max_booking_coverage <= 1)),
...
insert into public.reward_rules
(issuer,card_name,currency,partner,partner_type,redemption_value,redemption_currency,max_redeem_points,max_booking_coverage,source_url,effective_date,verified_at,notes,route_status,confidence,program_url,source_type)
values
('HDFC Bank','Diners Black Metal','HDFC Reward Points','HDFC SmartBuy flights & hotels','direct_travel',1.00,'INR',75000,0.70,'https://www.hdfcbank.com/content/bbp/repositories/723fb80a-2dde-42a3-9793-7ae1be57c87f/?path=%2FPersonal%2FPay%2FCards%2FCredit+Card%2FCredit+Card+Landing+Page%2FCredit+Cards%2FSuper+Premium%2FDiners+Club+Black+Metal+Edition%2FRewards-TnC-Diners-Black-Metal.pdf','2025-10-16',now(),'SmartBuy flights and hotels: ₹1 per Reward Point. Up to 70% of a booking may be paid with points, capped at 75,000 Reward Points per calendar month.','verified','high','https://offers.smartbuy.hdfcbank.com/','issuer'),
('ICICI Bank','Times Black','ICICI Reward Points','ICICI statement credit','statement_credit',0.40,'INR',null,null,'https://www.timesblack.com/faqs','2026-09-22',now(),'Statement-credit redemption: 1 ICICI Reward Point = ₹0.40. Products and vouchers may offer up to ₹1 per point, but need the live catalogue before a fixed value is presented.','verified','high','https://www.timesblack.com/allrewards','issuer');
...
-- Least-privilege table grants complement the row-level policies above.
