...
function HotelSearch(){
 ...
 const pointPanel=x=>{
   const routes=directRules.map(r=>{const card=wallet.find(w=>cardMatchesRule(w.card_name,r));if(!card)return null;const rate=Number(r.redemption_value||0);if(rate<=0)return null;const value=Math.min(Number(x.value||0),Number(card.points||0)*rate);const pointsUsed=value>0?Math.ceil(value/rate):0;return{card:card.card_name,value,pointsUsed,effective:rate,remaining:Math.max(0,Number(x.value||0)-value)}}).filter(Boolean);
   const best=routes.slice().sort((a,b)=>b.value-a.value)[0];
   ...
   return <div className="hotelPointPanel"><div><span className="pointEyebrow">POINTPILOT</span><strong>Pay cash vs use points</strong><small>{summary}</small></div><div className="pointDecision">{best?<><b>₹{Math.round(best.value).toLocaleString("en-IN")} saved</b><span>{best.pointsUsed.toLocaleString("en-IN")} pts • ₹{best.effective.toFixed(2)}/pt • ₹{Math.round(best.remaining).toLocaleString("en-IN")} cash left</span></>:<b>Cash route shown</b>}<span>Live hotel cash price: {formatRate(x)}</span></div></div>;
 };
