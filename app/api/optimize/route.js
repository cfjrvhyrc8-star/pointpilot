...
export async function POST(req){
  try{
    ...
    if(p.mode==="flight"){
      ...
      if(!travelportConfigured())return NextResponse.json({status:"not_configured",message:"Add the Travelport credentials to Vercel Environment Variables to enable live flight search."},{status:503});
      ...
      return NextResponse.json({status:"provider_error",provider:"Travelport",code:result.status,message:messageFor(result),trackingId:result.trackingId||"",data:result.data},{status:502});
    }
    if(p.mode==="hotel"){
      ...
      return NextResponse.json({status:result.data?.status||"provider_error",provider:"Travelport Stays",code:result.status,message:messageFor(result),trackingId:result.trackingId||"",data:result.data},{status:result.status===503?503:502});
    }
