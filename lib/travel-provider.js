...
async function requestStays(body){
  ...
  if(process.env.TRAVELPORT_STAYS_ACCESSGROUP){
    headers["XAUTH_TRAVELPORT_ACCESSGROUP"]=process.env.TRAVELPORT_STAYS_ACCESSGROUP;
  }else{
    headers["TVP-PCC-Core"]=process.env.TRAVELPORT_PCC;
  }

  const r=await fetch(TRAVELPORT_STAYS+"/search/searchcomplete/",{
    method:"POST",
    headers,
    body:JSON.stringify(body),
    cache:"no-store"
