...
async function getStaysToken(){
  ...
  const body=new URLSearchParams({grant_type:"client_credentials",client_id:process.env.TRAVELPORT_CLIENT_ID,client_secret:process.env.TRAVELPORT_CLIENT_SECRET,scope:"openid"});
  ...
  if(!r.ok)return{ok:false,status:r.status,data};
  staysTokenCache={token:data.access_token,expiresAt:Date.now()+Math.max(60,Number(data.expires_in||86400)-300)*1000};
  return{ok:true,token:data.access_token};
}
